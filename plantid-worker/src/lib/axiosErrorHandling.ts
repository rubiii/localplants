...
export function logError(error: unknown) {
  ...
  if (axiosError.response) {
    ...
    logger.error('Response status:', { status: response.status, statusText: response.statusText })
    ...
    logger.error('Failed to setup PlantNet request', { error: axiosError.message })
  }
}
